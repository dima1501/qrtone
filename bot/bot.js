...
const bot = new Telegraf(
  process.env.MODE == 'development'
    ? process.env.TELEGRAF_TOKEN_PROD
    : process.env.TELEGRAF_TOKEN_PROD,
  { polling: true }
);
  ...
  bot.action('like', async (ctx) => {
    try {
      const tgUser = await db
        .collection('tgUsers')
        .findOne({ _id: ctx.update.callback_query.message.chat.id });
      const user = await db
        .collection('users')
        .aggregate([
          ...
          {
            $match: {
              'notifications.chatId': { $in: [ctx.update.callback_query.message.chat.id] },
            },
          },
          {
            $match: {
              'notifications.messageId': { $in: [ctx.update.callback_query.message.message_id] },
            },
          },
          {
            $group: {
      ...
      if (user[0]) {
        for (let i = 0; i < user[0].list[0].messages.length; i++) {
          api.acceptFastActionTelegram({
            sockets: user[0].sockets[0],
            data: { _id: user[0].list[0]._id },
          });
    ...
    try {
      ...
      const user = await db
        ...
        .aggregate([
          ...
          {
            $match: {
              'messages.messageId': { $in: [ctx.update.callback_query.message.message_id] },
            },
          },
      ...
      if (user[0]) {
        for (let i = 0; i < user[0].list[0].messages.length; i++) {
          api.acceptOrderTelegram({
            sockets: user[0].sockets,
            orderId: user[0].list[0].orderId,
            guestId: user[0].list[0].guestId,
          });
          ctx.telegram.editMessageText(
            user[0].list[0].messages[i].chat.id,
            user[0].list[0].messages[i].message_id,
